...
MotionController::MotionController(MPU6500 &imu, MotorController &motor, WS2812 &led, PID &pid)
    : imu(imu), motor(motor), led(led), pid(pid), angle_x(0.0f), angle_y(0.0f), angle_z(0.0f),
      setpoint(0.0f), max_speed(1600), min_speed(650), last_time_ms(0) {}
...
void MotionController::update() {
    ...
    for (;;)
    {
        ...
        imu.updateAngles(angle_x, angle_y, angle_z, 0, 0, 0, last_wake_time);
        ...
        if (output_scaled > max_speed)
            output_scaled = max_speed;
        if (output_scaled < -max_speed)
            output_scaled = -max_speed;
